...
if (Fit_MC):
   ...
   # All_flatchain = np.vstack(All_chain[:,clen/4:])
	...
	colors = rainbow(np.linspace(0, 1, All_chain.shape[0]))
	for k in range(All_chain.shape[2]):
		plt.figure()
		#plt.figure()
		for i in range(All_chain.shape[0]):
			plt.plot(All_chain[i,:,k], drawstyle='steps', color='color[i]', marker=None, alpha=0.2)
	...
	###CORNER PLOT	
	All_flatchain = np.vstack(All_chain[:,clen*1/8:])
	All_flatlnprobs = np.vstack(All_lnprobs[:,clen*1/8:])
