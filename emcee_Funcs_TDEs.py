...
def ln_IR_fxdR_likelihood(p, t, THEargs1, THEargs2, RHStable, Ttable, RHS_mx, RHS_mn, y1, dy1, y2, dy2):
	return -(IRTDE_fxdR_Err2(p, t, THEargs1, THEargs2, RHStable, Ttable, RHS_mx, RHS_mn, y1, dy1, y2, dy2))
...
def IRTDE_Err2(p, t, argW1, argW2, RHStable, Ttable, RHS_mx, RHS_mn, y1, dy1, y2, dy2):
	...
	chiW2 = 0.5*( y2 - np.minimum(IRLC_point(p, t, argW2, RHStable, Ttable, RHS_mx, RHS_mn), 11.26) )/ np.sqrt( dy2*dy2 + p[5]*p[5]  )
...
def IRTDE_fxdR_Err2(p, t, argW1, argW2, RHStable, Ttable, RHS_mx, RHS_mn, y1, dy1, y2, dy2):
	...
	chiW2 = 0.5*(  y2 - np.minimum(IRLC_fxdR_point(p, t, argW2, RHStable, Ttable, RHS_mx, RHS_mn), 11.26)  )/ np.sqrt( dy2*dy2 + p[5]*p[5] )
